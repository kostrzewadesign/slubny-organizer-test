-- Fix security functions to allow system operations during user deletion
-- The main issue is that check_guest_operation_rate_limit blocks CASCADE DELETE operations

-- First, drop the existing problematic function
DROP FUNCTION IF EXISTS security.check_guest_operation_rate_limit();

-- Recreate the function with proper system operation handling
...
-- Also fix the enhanced_sanitize_guest_data function if it has similar issues
-- First check if it exists and recreate it properly
