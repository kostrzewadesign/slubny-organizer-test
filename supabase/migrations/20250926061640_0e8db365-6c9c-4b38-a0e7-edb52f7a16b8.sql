...
DROP FUNCTION IF EXISTS public.add_guest_with_companion(uuid, jsonb, jsonb);

-- Temporary RLS policy to allow guest insertion without authentication (with limits)
CREATE POLICY "temp_anonymous_guest_insert" ON public.guests
  FOR INSERT 
  TO anon
  WITH CHECK (
    -- Limit: no user_id required, but limit to 10 guests per hour per IP
    user_id IS NULL AND
    (SELECT COUNT(*) FROM public.guests 
     WHERE created_at > NOW() - INTERVAL '1 hour' 
     AND user_id IS NULL) < 10
  );

-- Recreate the add_guest_with_companion function to handle null auth.uid() gracefully
