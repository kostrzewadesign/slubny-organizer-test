-- Add missing fields to user_profiles first
ALTER TABLE public.user_profiles 
ADD COLUMN IF NOT EXISTS location text,
ADD COLUMN IF NOT EXISTS guest_count integer DEFAULT 50,
ADD COLUMN IF NOT EXISTS notes text;

-- Drop trigger, then function, then recreate both
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Recreate the handle_new_user function with fixes
...
BEGIN
  ...
  -- Create default budget categories from templates  
  BEGIN
