...
SET payment_status_new = CASE 
  WHEN payment_status = 'planned' THEN 'none'
  WHEN payment_status = 'Zaliczka' THEN 'deposit_paid'
  WHEN payment_status = 'Zapłacone' THEN 'paid'
  ELSE 'none'
END;
...
-- Drop old enum type
DROP TYPE IF EXISTS payment_status_old CASCADE;
...
BEGIN
  ...
  IF NEW.amount < OLD.deposit_amount AND NEW.payment_status = 'deposit_paid' THEN
